...
Mario.MapState.prototype.Enter = function() {
    ...
    this.WaterSprite.FramesPerSecond = 234;
    ...
    this.LargeMario.AddNewSequence("fire", 0, 7, 0, 5);
    ...
    this.Font = Mario.SpriteCuts.CreateCyanFont();
    ...
    var audio = document.getElementById("map");
...
Mario.MapState.prototype.Update = function(delta) {
        ...
        if (this.CanEnterLevel && Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.S)) {
            if (this.Level[x][y] === Mario.MapTile.Level && this.Data[x][y] !== -11) {
                if (this.Level[x][y] === Mario.MapTile.Level && this.Data[x][y] !== 0 && this.Data[x][y] > -10) {
                    difficulty = 1;
                    Mario.MarioCharacter.LevelString = "MANDOLORIAN-";
                    type = Mario.LevelType.Overground;
                
                 
                    if (this.Data[x][y] > 0 && ((Math.random() * 3) | 0) === 0) {
                        type = Mario.LevelType.Underground;
                   
                        Mario.MarioCharacter.LevelString = "FRINGE-";
                      
                    }
                    
                    if (this.Data[x][y] < 0) {
                        if (this.Data[x][y] === -2) {
                            var audio = document.getElementById("goodfight");
                            audio.volume = 1.0; audio.currentTime =0; 
                            var audio = document.getElementById("castle");
                            audio.volume = 0.0; audio.currentTime =0; 
                            var audio = document.getElementById("wtf");
                            audio.volume = 0.0; audio.currentTime =0; 
                            Mario.MarioCharacter.LevelString = "FINALE";
                            difficulty += 2;
                        } else if (this.Data[x][y] === -1) {
                            Mario.MarioCharacter.LevelString = "FOISTED!";
                            var audio = document.getElementById("wtf");
                            audio.volume = 1.0; audio.currentTime =0; 
    var audio = document.getElementById("goodfight");
    audio.volume = 0.0; audio.currentTime =0; 
    ...
    audio.volume = 0.0; audio.currentTime =0; 
                            ...
                            Mario.MarioCharacter.LevelString = "HELL";
                            difficulty = 1;
                            var audio = document.getElementById("castle");
                            audio.volume = 1.0; audio.currentTime =0; 
                            var audio = document.getElementById("goodfight");
...
Mario.MapState.prototype.TryWalking = function(xd, yd) {
    ...
    if (this.Level[xt][yt] === Mario.MapTile.Road || this.Level[xt][yt] === Mario.MapTile.Level) {
        ...
        this.XMarioA = xd * 8;
        this.YMarioA = yd * 8;
        var audio = new Audio('SMA4-MapMove.wav');
        audio.play();
...
Mario.MapState.prototype.Draw = function(context) {
    ...
    this.Font.Strings[0] = { String: "LIFE: " + Mario.MarioCharacter.Lives, X: 4, Y: 4 };
    this.FontShadow.Strings[0] = { String: "LIFE: " + Mario.MarioCharacter.Lives, X: 5, Y: 5 };
    this.Font.Strings[1] = { String: "DJ STUDIO " ,X: 210, Y: 4 };
    this.FontShadow.Strings[1] = { String: "DJ STUDIO ", X: 210, Y: 5 };
...
Mario.MapState.prototype.LevelWon = function() {
    ...
    if (this.Data[x][y] === -2) {
        ...
        setTimeout(function(){ window.location.replace("Yay.html");}, 0000);
        return;
    }
    if (this.Data[x][y] !== -3) {
        this.Data[x][y] = 0;
